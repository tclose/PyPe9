...
This module defines a function to calculate the overlapping pecentages of the forests, comparing to the whole box
@author: Haozhe Zhang
"""
...
def xy_overlaps(forest, VOX_SIZE, central_frac=(1.0, 1.0)):
        ...
        for z in range(mask._mask_array.shape[2]):
            ...
            temp=np.array(central_mask,dtype = int )  # transform the data type from bool to int
            flat_mask = np.sum(temp, axis=2)
            num_voxels =  float(np.prod(flat_mask.shape)) # number of voxels of the total "box"
