...
nest::RecordablesMap<nineml::IzhikevichReference> nineml::IzhikevichReference::recordablesMap_;

namespace nest{
  template <> void RecordablesMap<nineml::IzhikevichReference>::create() {
    insert_("U", &nineml::IzhikevichReference::get_y_elem_<nineml::IzhikevichReference::State_::U_INDEX>);
    insert_("V", &nineml::IzhikevichReference::get_y_elem_<nineml::IzhikevichReference::State_::V_INDEX>);
  }
}
...
extern "C" int IzhikevichReference_dynamics(double t, const double y_[], double f_[], void* pnode_) {

	// Get references to the members of the model
	assert(pnode_);
	const IzhikevichReference& node_ = *(reinterpret_cast<IzhikevichReference*>(pnode_));
	const IzhikevichReference::Parameters_& P_ = node_.P_;
	const IzhikevichReference::State_& S_ = node_.S_;
	const IzhikevichReference::Buffers_& B_ = node_.B_;

    if (B_.current_regime == IzhikevichReference::subthreshold_regime_REGIME) {
	    // State Variables
        const double_t& V = S_.y_[IzhikevichReference::State_::V_INDEX];  // (mV)
        const double_t& U = S_.y_[IzhikevichReference::State_::U_INDEX];  // (mV/ms)
	    ...
	    //std::cout << "U=" << a*(-U + V*b);
	    ITEM(f_, IzhikevichReference::State_::U_INDEX) = a*(-U + V*b);  // (mV/ms^2)
	    //std::cout << "V=" << -U + V*beta + alpha*(V*V) + zeta + Isyn/C_m;
	    ITEM(f_, IzhikevichReference::State_::V_INDEX) = -U + V*beta + alpha*(V*V) + zeta + Isyn/C_m;  // (mV/ms)
    }
...
extern "C" int IzhikevichReference_jacobian (double t, const double y[], double *dfdy, double dfdt[], void* pnode) {
    // cast the node ptr to an object of the proper type
    assert(pnode);
    const IzhikevichReference & node = *(reinterpret_cast<IzhikevichReference*>(pnode));
    IzhikevichReference & vnode =    *(reinterpret_cast<IzhikevichReference*>(pnode));

    // state is a reference to the model state
    struct IzhikevichReference::Buffers_ *b;
    b = &(vnode.B_);

    for (int i = 0; i < b->N; i++)
        b->u[i] = y[i] + 0.01;

    IzhikevichReference_dynamics(t, b->u, b->jac, pnode);
    for (int i = 0; i < b->N; i++)
...
IzhikevichReference::Parameters_::Parameters_():
    a (0.0),
    c (0.0),
    b (0.0),
    d (0.0),
...
IzhikevichReference::State_::State_(const Parameters_& p) {
 ...
 ***********************************/
IzhikevichReference::State_::State_(const State_& s) {
  for (int i = 0 ; i < 2 ; ++i)
      y_[i] = s.y_[i];
}
...
IzhikevichReference::State_& IzhikevichReference::State_::operator=(const State_& s) {
  assert(this != &s);
...
void IzhikevichReference::calibrate() {
    B_.logger_.init();
...
void IzhikevichReference::Parameters_::get (DictionaryDatum &d_) const {
...
void IzhikevichReference::Parameters_::set (const DictionaryDatum &d_) {
...
void IzhikevichReference::State_::get (DictionaryDatum &d_) const {
    // Get states from internal variables
    def<double_t>(d_, "U", y_[0]);
    def<double_t>(d_, "V", y_[1]);
}

void IzhikevichReference::State_::set (const DictionaryDatum &d_, const Parameters_&) {
...
IzhikevichReference::Buffers_::Buffers_(IzhikevichReference& n)
    : logger_(n),
      s_(0),
      c_(0),
      e_(0),
...
IzhikevichReference::Buffers_::Buffers_(const Buffers_&, IzhikevichReference& n)
    : logger_(n),
      s_(0),
      c_(0),
      e_(0),
...
void IzhikevichReference::init_buffers_() {
    ...
    B_.sys_.function  = IzhikevichReference_dynamics;
    B_.sys_.jacobian  = IzhikevichReference_jacobian;
...
IzhikevichReference::IzhikevichReference()
    : Archiving_Node(),
      P_(),
      S_(P_),
      B_(*this) {
...
IzhikevichReference::IzhikevichReference(const IzhikevichReference& n)
    : Archiving_Node(n),
      P_(n.P_),
      S_(n.S_),
      B_(n.B_, *this) {}

void IzhikevichReference::init_node_(const Node& proto) {
    const IzhikevichReference& pr = downcast<IzhikevichReference>(proto);
    P_ = pr.P_;
    S_ = State_(P_);
...
void IzhikevichReference::init_state_(const Node& proto) {
    const IzhikevichReference& pr = downcast<IzhikevichReference>(proto);
    S_ = State_(pr.P_);
...
void IzhikevichReference::update(nest::Time const & origin, const nest::long_t from, const nest::long_t to) {
    ...
    double f_[IzhikevichReference::State_::STATE_VEC_SIZE_];  // Vector to hold the time derivatives

    if (origin.get_ms() < 10) {
        std::cout << "State: ";
        for (int i = 0; i < IzhikevichReference::State_::STATE_VEC_SIZE_; ++i)
            std::cout << S_.y_[i] << ", ";
        std::cout << std::endl;
	        ...
	        if (B_.current_regime == subthreshold_regime_REGIME) {
	            ...
	            if (V > theta) {
	                ...
	                if (B_.subthreshold_regime_trigger_0_active) {
				        ...
				        // State assignments
				        S_.y_[IzhikevichReference::State_::U_INDEX] = U + d;  // (mV/ms)
				        S_.y_[IzhikevichReference::State_::V_INDEX] = c;  // (mV)
...
void IzhikevichReference::handle(nest::SpikeEvent & e) {
    assert(e.get_delay() > 0);
    // Loop through event receive ports
}

void IzhikevichReference::handle(nest::CurrentEvent& e) {
    assert(e.get_delay() > 0);
...
void IzhikevichReference::handle(nest::DataLoggingRequest& e) {
    B_.logger_.handle(e);
}
