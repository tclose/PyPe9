...
namespace nineml {
    ...
    class IzhikevichReference : public nest::Archiving_Node {

      public:

        ~IzhikevichReference();
	    IzhikevichReference(const IzhikevichReference &);
	    IzhikevichReference();
    ...
    // Set dynamics methods (the ones that actually model the dynamics) as friends
	    friend int IzhikevichReference_dynamics(double t, const double y_[], double f_[], void* pnode_);
	    ...
	    friend class nest::RecordablesMap<IzhikevichReference>;
	    friend class nest::UniversalDataLogger<IzhikevichReference>;
        ...
        struct Buffers_ {
	        Buffers_(IzhikevichReference&);
	        Buffers_(const Buffers_&, IzhikevichReference&);
	        nest::UniversalDataLogger<IzhikevichReference> logger_;
		...
		static nest::RecordablesMap<IzhikevichReference> recordablesMap_;
    
	}; // end class IzhikevichReference

    inline nest::port IzhikevichReference::send_test_event(nest::Node& target, nest::port receptor_type, nest::synindex, bool) {
		nest::SpikeEvent e;
		e.set_sender(*this);
		return target.handles_test_event(e, receptor_type);
    }

    inline nest::port IzhikevichReference::handles_test_event(nest::SpikeEvent&, nest::port receptor_type) {
    ...
    inline nest::port IzhikevichReference::handles_test_event(nest::CurrentEvent&, nest::port receptor_type) {
	...
	inline nest::port IzhikevichReference::handles_test_event(nest::DataLoggingRequest& dlr, nest::port receptor_type) {
	    if (receptor_type != 0)
            throw nest::UnknownReceptorType(receptor_type, this->get_name());
		return B_.logger_.connect_logging_device( dlr, recordablesMap_ );
    }

    inline void IzhikevichReference::get_status(DictionaryDatum &d) const {
		P_.get(d);
		S_.get(d);
    ...
    inline void IzhikevichReference::set_status(const DictionaryDatum &d) {
    ...
    inline float IzhikevichReference::random_uniform_(float low, float high) {
        return low + (high - low) * V_.rng_->drand();
    }

    inline float IzhikevichReference::random_normal_(float mu, float sigma) {
    ...
    inline float IzhikevichReference::random_exponential_(float lambda) {
